module Openkick
  class RecordData
    ...
    def search_data(method_name = nil)
      ...
      unless partial_reindex
        index.suggest_fields.each do |field|
          source[field] = nil if !source.key?(field) && !source.key?(field.to_sym)
        end
      end

      # locations
      index.locations_fields.each do |field|
        next unless source[field]

        source[field] = if !source[field].is_a?(Hash) && (source[field].first.is_a?(Array) || source[field].first.is_a?(Hash))
                          # multiple locations
                          source[field].map { |a| location_value(a) }
                        else
                          location_value(source[field])
                        end
      end

      source[:type] = document_type(true) if index.options[:inheritance] && !TYPE_KEYS.any? { |tk| source.key?(tk) }

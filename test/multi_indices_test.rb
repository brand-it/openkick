...
class MultiIndicesTest < Minitest::Test
  ...
  def test_index_name
    ...
    assert_equal [], Product.search("product", index_name: Speaker.openkick_index.name, conversions: false).map(&:name)
  end

  def test_models_and_index_name
    store_names ["Product A"]
    store_names ["Product B"], Speaker
    assert_equal ["Product A"], Openkick.search("product", models: [Product, Store], index_name: Product.openkick_index.name).map(&:name)

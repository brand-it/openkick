...
class HighlightTest < Minitest::Test
  ...
  def test_with_highlights
    ...
    assert_equal 'Two Door <em>Cinema</em> Club',
                 Product.search('cinema', highlight: true).with_highlights.first.last[:name]
  end

  def test_tag
    store_names ['Two Door Cinema Club']

    assert_equal 'Two Door <strong>Cinema</strong> Club',
                 Product.search('cinema', highlight: { tag: '<strong>' }).highlights.first[:name]
  end

  def test_tag_class
    store_names ['Two Door Cinema Club']

    assert_equal "Two Door <strong class='classy'>Cinema</strong> Club",
                 Product.search('cinema', highlight: { tag: "<strong class='classy'>" }).highlights.first[:name]
  end

  def test_very_long
    store_names [('Two Door Cinema Club ' * 100).strip]

    assert_equal ('Two Door <em>Cinema</em> Club ' * 100).strip,
                 Product.search('cinema', highlight: true).highlights.first[:name]
  end

  def test_multiple_fields
    store [{ name: 'Two Door Cinema Club', color: 'Cinema Orange' }]
    highlights = Product.search('cinema', fields: %i[name color], highlight: true).highlights.first
  ...
  def test_fields
    store [{ name: 'Two Door Cinema Club', color: 'Cinema Orange' }]
    highlights = Product.search('cinema', fields: %i[name color], highlight: { fields: [:name] }).highlights.first
  ...
  def test_field_options
    ...
    assert_equal 'Two Door <em>Cinema</em> Club are',
                 Product.search('cinema', highlight: { fields: { name: { fragment_size: } } }).highlights.first[:name]
  end
  ...
  def test_encoder
    ...
    assert_equal '&lt;b&gt;<em>Hello</em>&lt;&#x2F;b&gt;',
                 Product.search('hello', highlight: { encoder: 'html' }, misspellings: false).highlights.first[:name]
  end
  ...
  def test_word_middle
    ...
    assert_equal 'Two Door <em>Cinema</em> Club',
                 Product.search('ine', match: :word_middle, highlight: true).highlights.first[:name]
  end
  ...
  def test_body
    ...
    assert_equal 'Two Door <strong>Cinema</strong> Club', Product.search(body:).highlights.first[:'name.analyzed']
  end
  ...
  def test_multiple_highlights
    ...
    assert_kind_of Array, highlights
    assert_equal 2, highlights.count
    refute_equal highlights.first, highlights.last
    highlights.each do |highlight|
      assert_includes highlight, '<em>Cinema</em>'
    end
  end

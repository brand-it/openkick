...
class MatchTest < Minitest::Test
  ...
  def test_stemming_tokens
    assert_equal ['milk'], Product.openkick_index.tokens('milks', analyzer: 'openkick_search')
    assert_equal ['milk'], Product.openkick_index.tokens('milks', analyzer: 'openkick_search2')
  end
  ...
  def test_no_arguments
    ...
    assert_empty Product.search.to_a
  end
  ...
  def test_operator
    store_names %w[fresh honey]

    assert_search 'fresh honey', %w[fresh honey], { operator: 'or' }
    assert_search 'fresh honey', [], { operator: 'and' }
    assert_search 'fresh honey', %w[fresh honey], { operator: :or }
    assert_search 'fresh honey', %w[fresh honey], { operator: :or, body_options: { track_total_hits: true } }
  ...
  def test_fields_operator
    ...
    assert_search 'red blue', %w[red blue cyan magenta], operator: 'or', fields: ['color']
  end
  ...
  def test_fields_both_match
    ...
    assert_first 'blue', 'Blue B', fields: %i[name color]
  end
end

...
class PaginationTest < Minitest::Test
  ...
  def test_nil_page
    ...
    assert_predicate products, :first_page?
  end
  ...
  def test_deep_paging
    with_options({ deep_paging: true }, Song) do
      assert_empty Song.search('*', offset: 10_000, limit: 1).to_a
    end
  end
  ...
  def test_no_deep_paging
    ...
    error = assert_raises(Openkick::InvalidQueryError) do
      Song.search('*', offset: 10_000, limit: 1).to_a
    end
  ...
  def test_max_result_window
    Song.delete_all
    with_options({ max_result_window: 10_000 }, Song) do
      relation = Song.search('*', offset: 10_000, limit: 1)
  ...
  def test_search_after
    ...
    products = Product.search('product', body_options: { search_after: }, **options)

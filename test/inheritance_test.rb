...
class InheritanceTest < Minitest::Test
  ...
  def test_child_index_name
    assert_equal "animals_test#{ENV.fetch('TEST_ENV_NUMBER', nil)}", Dog.openkick_index.name
  end
  ...
  def test_missing_records
    ...
    record.delete
    assert_output nil, %r{\[openkick\] WARNING: Records in search index do not exist in database: Cat/Dog \d+} do

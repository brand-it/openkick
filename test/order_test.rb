...
class OrderTest < Minitest::Test
  def test_hash
    ...
    assert_order_relation ['Product D', 'Product C', 'Product B', 'Product A'],
                          Product.search('product').order(name: :desc)
  end
  ...
  def test_multiple
    ...
    assert_order_relation ['Product A', 'Product B', 'Product C'],
                          Product.search('product').order(color: :asc, store_id: :desc)
    assert_order_relation ['Product A', 'Product B', 'Product C'],
                          Product.search('product').order(:color, store_id: :desc)
    assert_order_relation ['Product A', 'Product B', 'Product C'],
                          Product.search('product').order(color: :asc).order(store_id: :desc)
    assert_order_relation ['Product B', 'Product C', 'Product A'],
                          Product.search('product').order(color: :asc).reorder(store_id: :desc)
  end
  ...
  def test_script
    store_names %w[Red Green Blue]
    order = { _script: { type: 'number', script: { source: "doc['name'].value.length() * -1" } } }

    assert_order('*', %w[Green Blue Red], order:)
    assert_order_relation %w[Green Blue Red], Product.search('*').order(order)
  end
end

...
class AggsTest < Minitest::Test
  def setup
    super
    store [
      { name: 'Product Show', latitude: 37.7833, longitude: 12.4167, store_id: 1, in_stock: true, color: 'blue',
        price: 21, created_at: 2.days.ago },
      { name: 'Product Hide', latitude: 29.4167, longitude: -98.5000, store_id: 2, in_stock: false, color: 'green',
        price: 25, created_at: 2.days.from_now },
      { name: 'Product B', latitude: 43.9333, longitude: -122.4667, store_id: 2, in_stock: false, color: 'red',
  ...
  def test_script
    source = "'Color: ' + _value"
    agg = Product.search('Product', aggs: { color: { script: { source: } } }).aggs['color']
  ...
  def test_ranges
    ...
    assert_in_delta(10.0, agg['buckets'][0]['to'])
    assert_in_delta(20.0, agg['buckets'][2]['from'])
  ...
  def test_where_override
    assert_equal ({}),
                 store_agg(where: { color: 'red' }, aggs: { store_id: { where: { in_stock: false, color: 'blue' } } })
    assert_equal ({ 2 => 1 }),
                 store_agg(where: { color: 'blue' }, aggs: { store_id: { where: { in_stock: false, color: 'red' } } })
  end
  ...
  def test_multiple
    assert_equal ({ 'store_id' => { 1 => 1, 2 => 2 }, 'color' => { 'blue' => 1, 'green' => 1, 'red' => 1 } }),
                 store_multiple_aggs(aggs: %i[store_id color])
  end

  def test_smart_aggs_false
    assert_equal ({ 2 => 2 }),
                 store_agg(where: { color: 'red' }, aggs: { store_id: { where: { in_stock: false } } },
                           smart_aggs: false)
    assert_equal ({ 2 => 2 }),
                 store_agg(where: { color: 'blue' }, aggs: { store_id: { where: { in_stock: false } } },
                           smart_aggs: false)
  end
  ...
  def test_aggs_with_time_zone
    start_time = Time.at(1_529_366_400)
    ...
    expected_london_buckets = [
      { 'key_as_string' => '2018-06-19T00:00:00.000+01:00', 'key' => 1_529_362_800_000, 'doc_count' => 3 }
    ]
    ...
    expected_sydney_buckets = [
      { 'key_as_string' => '2018-06-19T00:00:00.000+10:00', 'key' => 1_529_330_400_000, 'doc_count' => 1 },
      { 'key_as_string' => '2018-06-20T00:00:00.000+10:00', 'key' => 1_529_416_800_000, 'doc_count' => 2 }
    ]
  ...
  def test_aggs_avg
    ...
    assert_in_delta(16.5, products.aggs['avg_price']['value'])
  end
